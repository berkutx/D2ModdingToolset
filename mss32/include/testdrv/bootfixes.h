...
/** Read-only validation of every requested Russobit boot-patch site. The
 * caller supplies the already-parsed harness plan; this function never reads
 * environment variables and never changes executable memory. */
bool preflight(bool skipIntro, bool blackScreen);

/** Apply the plan accepted by preflight(). This runs only after the ordinary
 * MSS hook transaction has committed, so every impossible apply failure is
 * terminal; the function cannot report a recoverable failure to DllMain. */
void commit();
