...
$testOutput = Join-Path $testWorkspace '.diagnostics\ownedwindowtimer-tests'
...
$testBuildStart.Arguments = '"' + (Join-Path $PSScriptRoot 'ownedwindowtimer_tests.vcxproj') + '" /t:Build /p:Configuration=Release /p:Platform=Win32 /nologo /v:minimal'
...
if ($testBuildProcess.ExitCode -ne 0) { throw 'Owned-window timer isolated test build failed' }

$testExe = Join-Path $testRun 'ownedwindowtimer_tests.exe'
Copy-Item -LiteralPath (Join-Path $testOutput 'bin\ownedwindowtimer_tests.exe') -Destination $testExe
& $testExe | Tee-Object -FilePath (Join-Path $testRun 'result.log')
if ($LASTEXITCODE -ne 0) { throw 'Owned-window timer isolated tests failed' }
