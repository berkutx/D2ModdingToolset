...
if (!$BuildDirectory) {
    $BuildDirectory = Join-Path $taskRoot ('.diagnostics/timer-auto-drag-' + (Get-Date -Format 'yyyyMMdd-HHmmss-fff'))
}
...
$taskState += Get-SourceBlock 'enum NativeAutoBattleResult' '// Runs only on the game GUI thread.'
$taskState | Set-Content -LiteralPath (Join-Path $taskBuild 'timerhost-auto-state.generated.h') -Encoding utf8
$taskDispatch = Get-SourceBlock 'int dispatchForcedAutoBattleReady()' 'int nativeBattleAnimationState('
$taskDispatch | Set-Content -LiteralPath (Join-Path $taskBuild 'timerhost-auto-dispatch.generated.h') -Encoding utf8
$taskTest = Join-Path $PSScriptRoot 'timer_auto_drag_tests.cpp'
...
$taskCommand = 'call "{0}" && cl.exe /nologo /EHsc /O2 /W4 /I"{1}" "{2}" /Fe:timer_auto_drag_tests.exe /Fo:timer_auto_drag_tests.obj /link /MANIFEST:EMBED /MANIFESTUAC:"level=''asInvoker'' uiAccess=''false''"' -f $taskVcvars,$taskBuild,$taskTest
...
if ($taskProcess.ExitCode) { Get-Content -LiteralPath (Join-Path $taskBuild 'build.log'); throw 'Timer Auto drag replay compilation failed.' }
...
& (Join-Path $taskBuild 'timer_auto_drag_tests.exe') | Tee-Object -FilePath (Join-Path $taskBuild 'test.log')
if ($LASTEXITCODE) { throw 'Timer Auto drag replay checks failed.' }
