...
| `C4menu.ini` | generated by the menu on first launch | gameplay toggles, Fast AI, language and diagnostics; the internal `wideBattle` flag has no menu item in the current release |
...
| 60 | ~170 ms | low |
| 100 | ~100 ms | moderate |
| 180 (shipped) | ~56 ms | higher |
| -1 (uncapped) | as fast as the CPU allows | one core busy |

The old DisciplesGL "ColdCPU" feel corresponds to 30. The shipped default is 180: fast command
processing at a bounded CPU cost. The in-game menu changes this limiter live and resets its old
deadline. Note that `maxfps` has no such effect: it paces only the render thread.
...
| MNS/SMNS > Map Scroll > Left / Middle Mouse Button | independent checkboxes, both enabled by default. Hold the enabled button and drag to pan 1:1 from the exact press point. A left click without movement still selects an object; middle-button dragging does not select. Disabling a button restores its native handling. Each missing preference inherits the old `dragScroll` value (default: on). Edge scrolling has its own checkbox and pauses during a drag | `C4menu.ini` `dragScrollLeft`, `dragScrollMiddle` | live |
| MNS/SMNS > Map Scroll > Edge Detection | scroll when the pointer reaches the map edge; enabled by default, independently of both drag buttons. Follows the game's Fast / Normal / Slow setting (`ScrollSpeed=0/50/100`) using the resolution-scaled DisciplesGL cadence | `C4menu.ini` `edgeScroll`; speed in `Disciple.ini` | live |
...
| (MNS/SMNS) Map animation speed: Off (default) / 1.5x .. 15x | the same virtual clock on the strategic map (water, flags, effects). `Ctrl +/-` adjusts the visible map/battle value by 0.1x and prints it in game; main and numpad keys are supported | `C4menu.ini` `mapAnimEnabled` + `mapAnimSpeed` + exact `*Factor` | live |
...
Widescreen Battle is intentionally absent from the current menu. On a recognized original layout it
...
| `C4menu.ini` | создаётся меню при первом запуске | игровые тумблеры, Fast AI, язык и диагностика; внутренний флаг `wideBattle` не имеет пункта меню в текущем релизе |
...
| 60 | ~170 мс | низкий |
| 100 | ~100 мс | умеренный |
| 180 (в комплекте) | ~56 мс | выше |
| -1 (без капа) | насколько хватит CPU | одно ядро занято |

Старому «ColdCPU» из DisciplesGL соответствует 30. В комплекте 180: быстрые реакции при
ограниченной цене по CPU. Меню меняет этот лимитер сразу и сбрасывает старый дедлайн. Заметьте:
`maxfps` такого эффекта не имеет, он крутит только поток рендера.
...
| MNS/SMNS > Прокрутка карты > Левая / Средняя кнопка мыши | независимые галочки, обе включены по умолчанию. Зажмите включённую кнопку и тяните карту: движение 1:1 от точной точки нажатия. Левый клик без движения выбирает объект; перетаскивание средней кнопкой не выбирает. Отключение кнопки возвращает её штатную обработку. Каждый отсутствующий ключ наследует прежний `dragScroll` (по умолчанию включено). Прокрутка у края имеет отдельную галочку и приостанавливается во время перетаскивания | `C4menu.ini` `dragScrollLeft`, `dragScrollMiddle` | сразу |
| MNS/SMNS > Прокрутка карты > У края экрана | прокрутка при наведении на край карты; по умолчанию включена независимо от обеих кнопок. Следует игровой настройке Быстро / Нормально / Медленно (`ScrollSpeed=0/50/100`) с масштабируемым по разрешению тактом DisciplesGL | `C4menu.ini` `edgeScroll`; скорость в `Disciple.ini` | сразу |
...
| (MNS/SMNS) Скорость анимаций карты: Выкл (по умолчанию) / 1.5x .. 15x | те же виртуальные часы на стратегической карте (вода, флаги, эффекты). `Ctrl +/-` меняет видимую карту/бой с шагом 0,1x и показывает значение в игре; работают основные клавиши и numpad | `C4menu.ini` `mapAnimEnabled` + `mapAnimSpeed` + точный `*Factor` | сразу |
...
«Широкий бой» намеренно отсутствует в текущем меню. На распознанной исходной раскладке он остаётся
