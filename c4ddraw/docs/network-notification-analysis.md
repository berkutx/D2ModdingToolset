...
|---|---|
| [Состояние и гонка](../tests/run-netnotifystate-tests.ps1) | 13 сценариев: production WakeState, шесть извлечённых без изменения методов MSS, настоящие потоки и Win32 post/dispatch; очередь восстанавливается после 1024 безрезультатных проверок. FIFO и доступ к сервису заданы стендом |
| [Native adapter](../tests/run-netnotify-adapter-tests.ps1) | 18 сценариев / 211 проверок: production netnotify + messagebatch, настоящие HWND/сообщения, modal defer/requeue, lifecycle, SEH, worker shutdown; оригинальные функции игры заменены стендом. Реальный Detours и фактический iteration thunk на частном synthetic site сохраняют ABI |
| [Владение таймером](../tests/run-ownedwindowtimer-tests.ps1) | 27 проверок на Win32: одинаковый ID на разных HWND, старые сообщения, смена поколения, teardown и освобождение из callback |
| [Конечная модель](../tests/model-net-notification.py) | 11 конфигураций: конечные повторы имеют контрпример; возобновляемые попытки дают продвижение при явно заданных условиях |
...
В CI эти стенды запускаются вместе с существующими проверками таймера, палитры и сетевой диагностики. Они проверяют реализацию механизма, но не измеряют частоту чужого сбоя и не заменяют сетевой прогон игры. Подробности событий 240–244 и их ограничения приведены в [NETWORK_TRACE.md](../NETWORK_TRACE.md).
