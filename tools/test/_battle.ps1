...
# _relay.ps1 (this uses Get-World/Get-Stacks/Move-StackToward/Invoke-Toggle/Get-Dialog/Invoke-Button).
...
function Invoke-HeroAttack {
    ...
    # use that transient map as evidence that the turn is active and do not probe/refire Move-StackToward.
    ...
    # reporter's observation of the exact stock turn + object-lock admission used by Move-StackToward.
    ...
    if (-not $RequireFreeSelfStack) {
        if (-not (Move-StackToward $Role $hero.id ($ax + 5) ($ay + 5))) {
    ...
    # Attack: Move-StackToward onto the monster's tile -> routed adjacent, end=monster -> the server starts the battle.
    if (-not (Move-StackToward $Role $hero.id $mon.x $mon.y)) { return [pscustomobject]@{ ok = $false; reason = "attack move was not issued" } }
