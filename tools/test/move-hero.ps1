...
try {
    ...
    # the hero and the exact stock strategic admission. Do not probe or retry Move-StackToward.
    ...
    if ($hero.inside) {
        $heroId = $hero.id
        if (-not (Move-StackToward host $heroId ([int]$hero.x + 5) ([int]$hero.y + 5))) {
    ...
    if (-not (Move-StackToward host $hero.id $tx $ty)) { throw "move was not issued (Move-StackToward returned found=false)" }
