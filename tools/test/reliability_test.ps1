...
# Headless boot reliability: launch the self-nav 'exit' client N times and confirm each reaches
# DLG_MAIN_MENU (counted from the shared mss32.log). Fails the run unless every launch reached it.
param([int]$N = 20, [int]$TimeoutSec = 45, [string]$Boot = "", [string]$Activate = "", [int]$SettleMs = 2500, [switch]$Keep, [string]$GameDir)
...
$exe = "$GameDir\Discipl2.exe"
$log = "$GameDir\mss32.log"
...
for ($i = 1; $i -le $N; $i++) {
    ...
    $psi.EnvironmentVariables["D2TESTDRV_SELFNAV"] = "1"            # run the built-in exit script
    $psi.EnvironmentVariables["D2TESTDRV_BLACKSCREEN_FIX"] = $Boot
    $t0 = Get-Date
...
if ($Keep) {
    ...
    Write-Output ("Kept last instance running (pid={0}), foregrounded. Close with: Stop-Process -Id {0}" -f $p.Id)
} else {
