...
# Existing clients and DirectPlay helpers belong to other sessions. This lobby
# test owns only the Process objects returned by the two launch calls below.
...
try {
    $client = Start-GameClient -GameDir $GameDir -Role host -Transport Lobby
    ...
    Start-Sleep 1
    if (-not (Set-SecretEditText host DLG_LOGIN_ACCOUNT EDIT_PASSWORD $Password)) { throw "EDIT_PASSWORD not set" }
    Start-Sleep 1
    ...
    if ($Keep) {
        ...
        if ($client) { Stop-OwnedProcess $client }
        if ($relay)  { Stop-OwnedProcess $relay }
    }
}
if (-not $ok) { exit 1 }
