...
Current branch: [one harness, Local/Lobby adapters and build switches](../../docs/TEST_HARNESS.md).
The fixed-fixture 18-case suite remains Local; generated Lobby bootstrap is a
separate result boundary. Use Release with `D2TestDrv=true`, not a ReleaseTest configuration.
The CI/video sections below document the reusable donor tooling, not an assertion
that this branch runs its old full game matrix. This PR's workflow builds the DLL
variants and runs offline contracts; live integration requires the explicit commands
and prerequisites in the current-branch guide above.
...
and text input) through a local relay process. Tests assert against live interface/world state;
the simultaneous-turn suite also checks native causal telemetry. Screenshots and recordings are
supporting evidence, not a substitute for those assertions.
...
A finite run binds each role/PID to one connection; disconnect is terminal, not an automatic reconnect.
...
Paired automatic startup (`D2TESTDRV_SCRIPTED_POPUPS=1`, with `RELAY_BRIDGE`) waits for both
exact host/join clients to publish `mapLoaded=true`. This means the native scenario and phase
exist, independently of active turn or open modals. While `startupActionsHeld=true`, only briefing
navigation may be auto-confirmed. The relay sends one `ReleaseStartupActions` (`0x0312`, empty
payload) to each client; `/api/status.startupActionsRelease` retains the PID/UI-sequence proof.
No OH controller signal, timer expiry or reconnect can release this initial-load barrier.
...
Stack combat profiles (`slots[].reach`, `atkClass`, `xp`, max `hp`, `armor`, etc.) additionally
require `D2TESTDRV_WORLD_PROFILES=1`. These getters can execute Lua modifiers for every unit, so
ordinary movement/battle tests do not request them. Slot occupancy, current unit HP, movement,
resources and camp rosters remain available with `WORLD` alone. `luckytest-arena.ps1` requests
`WORLD_PROFILES` automatically; with `-AttachHost`, set it when launching the attached clients.
...
`strategicActionReady` is the world reporter's UI-thread observation of the production action
admission used by `Move-Stack`, including the active turn mode and phase object-lock predicate.
The harness observes that decision; it does not own the turn calendar. The relay exposes the same
...
## Simultaneous-turn acceptance

There is **one optional `D2_TESTDRV` harness and one test-control relay**,
[`tools/relay/relay.js`](../relay/relay.js), shared by the existing template, battle,
arena and simultaneous-turn tests. The production
[`simturns-relay`](../simturns-relay/README.md) is the coordinator under test: it
owns game turn scheduling, not UI commands, and is not a second test harness.

The [OH test guide](SIMTURNS.md) lists the full 18-cold-run campaign, all five
long-move cases, long-attack, protocol, relay-failure and post-merge checks, plus
offline commands and fixture requirements. Historical green runs are not a fresh
pass of the current candidate; each run must identify its actual DLL and result.
The automatic workflow includes offline contracts. Licensed-game OH acceptance
is a separate explicit local run, not a claim made by the ordinary multiplayer CI jobs.
...
| `simturns-acceptance-campaign.ps1` | 5 canonical + 1 battle-block + 12 independently cold ordered runs; validates every child summary; `-StopOnFailure` stops after the first failed child's teardown |
| `simturns-production-poc.ps1` | one real two-client OH run; [all modes and options](SIMTURNS.md) |
| `_simturns_gameplay.ps1`, `_legacy_mass_oracle.ps1`, `_literal_inner_startup.ps1`, `_owned_process_diagnostics.ps1` | scenario assertions, historical topology, startup evidence and exact-owned process diagnostics; use the same `_relay.ps1` |
| `fixtures/simturns-russobit.json`, `fixtures/stock-startup-*.json` | pinned OH executable/map and stock-startup contracts; no game binaries included |
| `auto-battle-http-contract.ps1`, `literal-inner-startup-test.ps1`, `legacy-mass-oracle-test.ps1` | offline contracts; do not launch the game |
| `prove-russobit-ui-bind-sites.py` | exact-executable binding proof; requires the matching local executable, not a normal CI fixture |
...
живое состояние интерфейса/мира; ОХ-сценарии также проверяют native causal telemetry.
Скриншоты и видео — дополнительные свидетельства, а не замена этим проверкам.
...
control endpoint каждого рилея. Native TCP-клиент разрешает DNS/имена Docker services.
В конечном прогоне роль/PID привязаны к одному соединению; disconnect завершает прогон ошибкой,
а не вызывает автоматическое переподключение.
...
Парный автозапуск (`D2TESTDRV_SCRIPTED_POPUPS=1` вместе с `RELAY_BRIDGE`) ждёт
`mapLoaded=true` от обоих точных host/join PID. Это наличие штатной карты и игровой фазы,
независимое от активного хода и открытых окон. При `startupActionsHeld=true` автоматически
закрывается только брифинг. Затем рилей однократно отправляет каждому `ReleaseStartupActions`
(`0x0312`, пустое тело); `/api/status.startupActionsRelease` сохраняет PID и номера UI-публикаций.
Барьер начальной загрузки не открывается по таймауту, reconnect или сигналу ОХ-контроллера.
...
Боевые характеристики (`slots[].reach`, `atkClass`, `xp`, максимальные `hp`, `armor` и др.)
дополнительно требуют `D2TESTDRV_WORLD_PROFILES=1`: их чтение может вызывать Lua-модификаторы
для каждого юнита. Обычные тесты движения и боя их не запрашивают. Слоты, текущие HP юнитов,
очки движения, ресурсы и состав лагерей доступны с одним `WORLD`. `luckytest-arena.ps1`
включает `WORLD_PROFILES` сам; при `-AttachHost` задайте флаг при запуске подключаемых клиентов.
...
`strategicActionReady` — наблюдение UI-потока за production admission, который использует
`Move-Stack`, с учётом текущего режима ходов и object-lock фазы. Harness наблюдает это решение,
но не владеет календарём ходов. Рилей отдаёт тот
...
## Приёмка одновременных ходов

Используется **один необязательный `D2_TESTDRV` harness и один управляющий тестовый
relay** — [`tools/relay/relay.js`](../relay/relay.js), общий для шаблонов, боёв,
арены и ОХ. Production [`simturns-relay`](../simturns-relay/README.md) — проверяемый
координатор самой игры, владелец расписания ходов, а не вторая тестовая система.

[Руководство по ОХ-тестам](SIMTURNS.md) перечисляет полный план из 18 холодных
прогонов, пять вариантов long-move, long-attack, protocol, потерю relay и проверки
после merge. Исторический зелёный результат не означает свежий PASS текущего
кандидата: для нового запуска нужны собственные DLL provenance и summary.
Offline-контракты входят в CI; живая ОХ-приёмка требует отдельного явного запуска
на точной лицензированной игре. Обычные multiplayer CI jobs её не заменяют.
...
| `simturns-acceptance-campaign.ps1` | 5 canonical + 1 battle-block + 12 отдельных холодных ordered-прогонов; строгая проверка каждого summary; `-StopOnFailure` останавливает кампанию после завершения и cleanup первого неуспешного случая |
| `simturns-production-poc.ps1` | один настоящий двухклиентный ОХ-прогон; [все режимы и параметры](SIMTURNS.md) |
| `_simturns_gameplay.ps1`, `_legacy_mass_oracle.ps1`, `_literal_inner_startup.ps1`, `_owned_process_diagnostics.ps1` | сценарные проверки, историческая топология, startup evidence и диагностика собственных процессов; общий `_relay.ps1` |
| `fixtures/simturns-russobit.json`, `fixtures/stock-startup-*.json` | закреплённые контракты EXE/карты и stock-startup, без игровых бинарников |
| `auto-battle-http-contract.ps1`, `literal-inner-startup-test.ps1`, `legacy-mass-oracle-test.ps1` | offline-контракты без запуска игры |
| `prove-russobit-ui-bind-sites.py` | проверка bindings на точном локальном EXE; не обычная CI-фикстура |
