...
try {
    ...
    $payloadWriter.Write([uint32]7) # single public protocol version
    ...
    $worldJson = '{"day":1,"activePlayerId":"0x00000000","strategicActionReady":true,"players":[],"stacks":[],"camps":[],"bags":[]}'
    ...
    $frameWriter.Write([uint16]0x0413) # LobbyChat
    $frameWriter.Write([uint16]0)
    ...
    # Publish one exact ready owner before the strict command mutation.
    $uiJson = '{"dialog":"DLG_PROTOCOL","instance":1,"ready":true,"strategicIdle":false,"mapLoaded":false,"startupActionsHeld":false,"widgets":[{"name":"TLBOX_PROTOCOL","type":"listbox","state":{"total":3,"selected":0}}],"targets":[{"dialog":"DLG_PROTOCOL","instance":42,"widgets":[{"name":"TLBOX_PROTOCOL","type":"listbox","state":{"total":3,"selected":0}}]}]}'
    $uiPayload = [Text.Encoding]::UTF8.GetBytes($uiJson)
    $frameWriter.Write([uint32](4 + $uiPayload.Length))
    $frameWriter.Write([uint16]0x0410)
    $frameWriter.Write([uint16]0)
    $frameWriter.Write($uiPayload)
    $frameWriter.Flush()
    if (-not (Wait-Dialog $roleText DLG_PROTOCOL 5)) { throw 'ready UI was not published' }
    ...
    $commandUri = "$script:RelayBase/api/ui/select?role=$([uri]::EscapeDataString($roleText))&dlg=DLG_PROTOCOL&lb=TLBOX_PROTOCOL&index=2&appearance=1&instance=42&timeoutMs=30000"
    ...
    $appearance = [BitConverter]::ToUInt32($command, 8)
    $owner = [BitConverter]::ToUInt32($command, 12)
    if ($appearance -ne 1 -or $owner -ne 42) { throw 'command lost its exact appearance/owner' }
    $offset = 16
