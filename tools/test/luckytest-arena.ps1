...
# Preserve the generic runner's queued UI-callback deadline; strict acceptance defaults stay 5s.
...
function Build-RoleSquad([string]$role) {
    ...
    if (-not $ldr -or $null -eq $ldr.PSObject.Properties['reach']) {
        throw "[$role] squad selection requires leader combat telemetry; launch the client with D2TESTDRV_WORLD_PROFILES=1 (also with -AttachHost)"
    }
    ...
    foreach ($n in $route) {
        ...
        if ($n.type -eq 'chest') {
            $null = Move-StackToward $role $hero.id $n.x $n.y
        ...
        $null = Move-StackToward $role $hero.id $enX $enY
        ...
        for ($s=0; $s -lt 4 -and -not $opened; $s++) {
            if (-not (Move-StackToward $role $hero.id $trX $trY)) { break }
...
try {
  if (-not $AttachHost) {
    ...
    $flags = @('SKIP_INTRO','BLACKSCREEN_FIX','UI_REPORTER','WORLD','WORLD_PROFILES','RELAY_BRIDGE')
    ...
    if ($CampProbe) {
        ...
        foreach ($camp in $myCamps) {
            ...
            $r = Move-StackToward host $hero.id $ex $ey
            Start-Sleep -Milliseconds 1500
            $dlg = Get-Dialog host
    ...
    if ($BuildSquad) {
        ...
        #    delivers the attack (Move-StackToward onto the enemy tile -> server starts the PvP battle = DLG_BATTLE_A).
        ...
        for ($cyc = 0; $cyc -lt 8 -and -not $battleRole; $cyc++) {
            ...
            if ($hp -and $jp) {
                ...
                $null = Move-StackToward host $hostHero ([int]$jp.x) ([int]$jp.y)
            ...
            if ($JoinerSelf) {
                ...
                if ($hp -and $jp) {
                    ...
                    $null = Move-StackToward join $joinHero ([int]$hp.x) ([int]$hp.y); Start-Sleep 2
    ...
    while (-not $SkipChests) {
        ...
        $mvBefore=$h.movement
        $null = Move-StackToward join $hero.id $b.x $b.y
        Start-Sleep -Milliseconds 1500
    ...
    if ($BisectCamp) {
        ...
        # OPEN (no BTN_BACK) for manual inspection.
        $h = @(Get-Stacks join) | Where-Object { $_.id -eq $hero.id } | Select-Object -First 1
        ...
        for ($s=0; $s -lt 8 -and -not $opened; $s++) {
            if (-not (Move-StackToward join $hero.id $camp.x $camp.y)) { Write-Host "[lt][BISECT] move rejected (turn over?)" -ForegroundColor Yellow; break }
    ...
    while ($true) {
        ...
        for ($s=0; $s -lt 6 -and -not $opened; $s++) {
            if (-not (Move-StackToward join $hero.id $camp.x $camp.y)) { $rejected=$true; break }
