...
.DESCRIPTION
  ...
  Every step uses the same native paths a real player drives, no input emulation:
    ...
    2. Exit the garrison. A hero starting INSIDE its capital reports the fort ANCHOR, not its real
       tile, and exiting is a free 0-cost step the game applies specially, so Move-StackToward to anchor+5
    ...
    5. Attack: Move-StackToward the hero onto the monster's tile. moveStack routes adjacent and sets the
