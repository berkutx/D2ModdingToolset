 ...
 *
 * A dependency-free Node.js server the mss32 bridge connects to over a Windows named pipe or,
 * for headless Wine/WSL runs, the same framed protocol over TCP.
 * MULTI-CLIENT (host + joiner, each tagged by role from its Hello), a dumb mirror + command
 * relay with no test logic: it mirrors each agent's live UI/world state and relays the
 * dispatcher's invoke/select/spin/edit/world commands to a role.
...
function handleMessage(socket, op, flags, payload) {
    ...
    case Op.UiSnapshot: {
        ...
        if (c) {
            ...
            if (r) {
                r.dialog = dialog; r.widgets = widgets;
    ...
    default:
        console.warn(`[agent] ${roleOf(socket)} unhandled op 0x${op.toString(16)} (${payload.length}B)`);
        break;
    }
}
...
// Agent connections use the Windows named pipe locally or the same framed protocol over TCP for
// Wine/WSL headless runners that cannot reach the Windows pipe.
function onAgentConn(socket) {
    console.log('[conn] client connected');
    state.clients.set(socket, { role: '?', pid: 0 });
    attachParser(socket);
    const drop = () => {
        const c = state.clients.get(socket);
